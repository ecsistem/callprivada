...
import { listDownsells, deletePresell, type PresellPage } from '../services/presellService';
import {
  Plus, ExternalLink, Edit3, Trash2, Copy, Check,
  TrendingDown, LayoutTemplate,
} from 'lucide-react';
...
const TEMPLATE_LABELS: Record<string, string> = {
  espera: '⚠️ Espera!',
  'ultima-chance': '🚨 Última Chance',
  fomo: '😱 FOMO',
  reconsidera: '💬 Reconsidera',
};

export default function DownsellsPage() {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      setDeletingId(null);
      qc.invalidateQueries({ queryKey: ['downsells'] });
    },
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-2xl font-bold text-white">Downsell Pages</h1>
            {pages.length > 0 && (
              <span className="flex items-center gap-1 text-xs text-red-400 bg-red-500/10 px-2 py-0.5 rounded-full font-medium">
          ...
          <p className="text-gray-500 text-sm mt-0.5">
            Página de recuperação — exibida para o lead que saiu ou recusou antes de pagar
          </p>
        </div>
        <Link to="/downsell/new"
          className="flex items-center gap-2 bg-red-700 hover:bg-red-600 text-white text-sm font-semibold px-4 py-2.5 rounded-xl transition-all shadow-lg shadow-red-900/30">
          <Plus size={16} />
          Nova downsell
        </Link>
      </div>
      ...
      {/* Tip */}
      <div className="flex items-start gap-3 bg-red-500/5 border border-red-500/15 rounded-2xl px-4 py-3.5">
        <TrendingDown size={15} className="text-red-400 mt-0.5 shrink-0" />
        <p className="text-xs text-gray-400 leading-relaxed">
          <span className="text-white font-medium">Como usar:</span> Coloque o link da downsell como destino do botão "fechar" ou "recusar" no overlay de pagamento da chamada. O lead que sair vê uma última oferta antes de ir embora.
        </p>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-white font-semibold text-lg mb-2">Nenhuma downsell criada ainda</p>
          <p className="text-gray-500 text-sm mb-7 max-w-sm leading-relaxed">
            Crie sua primeira página de downsell e recupere leads que estavam prestes a sair sem comprar.
          </p>
          <Link to="/downsell/new"
            className="flex items-center gap-2 bg-red-700 hover:bg-red-600 text-white text-sm font-semibold px-5 py-3 rounded-xl transition-all shadow-lg shadow-red-900/30">
            <Plus size={16} />
            Criar primeira downsell
          </Link>
        </div>
      ) : (
        <div className="space-y-3">
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex items-start gap-4 flex-1 min-w-0">
                  <div className="w-10 h-10 rounded-xl bg-red-500/10 flex items-center justify-center shrink-0">
                    <LayoutTemplate size={18} className="text-red-400" />
                  </div>
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 mt-2.5 flex-wrap">
                      ...
                      <CopyButton text={`${window.location.origin}/d/${page.slug}`} />
                      <a href={`/d/${page.slug}`} target="_blank" rel="noreferrer"
